...
def model_assignment(results, genes):
    ...
    cadds = [0,5,10] # the CADD 16 threshold has too little variants for the model to make sense
...
def prep_pl_for_skat(mts):
    ...
    for idx, mt in enumerate(mts):
        ...
        if os.path.exists('/net/ascratch/people/plggosborcz/ann_mts/_pl_'+str(idx)+'ann.mt'):
            
            mts[idx] = hl.read_matrix_table('/net/ascratch/people/plggosborcz/ann_mts/_pl_'+str(idx)+'ann.mt')
            ...
            mt = mt.checkpoint('/net/ascratch/people/plggosborcz/ann_mts/_pl_'+str(idx)+'ann.mt')
...
def run_skat_log(mtx, pcs, field, suffix):
    ...
    skat_table = skat_table.naive_coalesce(10)
